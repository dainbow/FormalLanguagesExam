...
\title{Коллоквиум по формальным языкам}
...
\begin{definition}
	Алфавит - непустое конечное множество, элементы которого называются символами. Обозначение: $\Sigma$.
\end{definition}
...
\begin{note} Дополнительные обозначения.
	...
	\begin{itemize}
		...
		\item Формальный язык $L \subset \Sigma^*$.
...
\begin{definition}
	Недетерминированный конечный автомат -- кортеж $M = \langle Q,\,\Sigma,\, \Delta,\, q_0,\, F \rangle$, где:
	\begin{itemize}
...
\begin{definition}
	Конфигурация в автомате $M = \langle Q,\,\Sigma,\, \Delta,\, q_0,\, F \rangle$ -- элемент $\langle q,\, w\rangle \in Q \times \Sigma^*$.
\end{definition}
...
\subsection*{Автоматные языки: примеры автоматных языков}
\begin{definition}
...
\begin{definition}
	Язык $L$ называется автоматным, если существует такой НКА $M$, что $L = L(M)$
\end{definition}
...
\subsection*{Различные варианты определений (упрощения НКА)}
\begin{proposition}
...
\begin{proof}
	...
	Значит
	\[
		\exists q_{m - 1}' :\: \Delta(q_{m - 1}) = q_m',\, \langle q_m',\, w_m\rangle \to q_m \Rightarrow \langle q_{m - 1},\, w_m\rangle \vdash_M \langle q_m,\, \varepsilon\rangle
	\]
...
\section{Про ДКА.}
\subsection*{Детерминированные конечные автоматы (ДКА)}
\begin{definition}
...
\subsection*{Эквивалентность ДКА и НКА}
\begin{theorem}
...
\section{Про автоматные языки}
\subsection*{Свойства класса автоматных языков. Замкнутость относительно булевых операций.}
\begin{definition}
...
\section{Про регулярные выражения}
...
\begin{definition}
	Регулярный автомат -- НКА, в котором на рёбрах записаны регулярные выражения. Докажем утверждение для регулярных автоматов.
\end{definition}
...
\subsection*{Алгоритм построения регулярного выражения по регулярному автомату.}
\begin{itemize}
	\item База:
...
\end{itemize}
\section{Про эквивалентные состояния ПДКА}
\subsection*{Эквивалентность состояний в ПДКА}
\begin{definition}
...
\subsection*{Эквивалентность слов по языку}
\begin{definition}
...
\subsection*{Оценка на минимальное количество состояний в ПДКА.}
\begin{lemma}
...
\section{Критерий минимальности количества состояний в ПДКА}
\begin{lemma}
...
\begin{proof}
	Пусть $M$ -- минимальный ПДКА. Строим автомат $Q / \sim_M$: уменьшаем число состояний. Если в $M$ есть недостижимое состояние, то удаляем его.

	Тогда из того, что в $M$ нет эквивалентных состояний:
	\begin{align*}
		\forall w_1,\, w_2 \in \Sigma^* :\: \Delta(q_0,\, w_1) \neq \Delta(q_0,\, w_2) \Rightarrow \exists u :\: \Delta(\Delta(q_0,\, w_1),\, u) \not\in F,\, \Delta(\Delta(q_0,\, w_2),\, u) \in F \Rightarrow \\
...
\section{Про канонический ПДКА}
\begin{definition}
	$M_1$ и $M_2$ изоморфны, если существует биекция $\psi:\: Q_1 \to Q_2$:
	\begin{itemize}
...
\subsection*{Канонический ПДКА - корректность построения}
...
\subsection*{Единственность минимального ПДКА}
\begin{lemma}
...
\begin{proof}
	...
	Докажем биективность $\psi$: $\Sigma^*/\sim_L = \vert Q_M\vert \Rightarrow$ достаточно доказать инъективность:
	\begin{align*}
...
\section{Алгоритмы проверок}
\subsection*{Алгоритм проверки МПДКА на эквивалентность}
...
\section{Лемма о разрастании}
\subsection*{Лемма о разрастании для автоматных языков}
\begin{lemma}
...
\section{Про порождающие грамматики}
\subsection*{Порождающие грамматики}
\begin{definition}
	Порождающая грамматика $G = \langle N,\, \Sigma,\, P,\, S\rangle$, где:
	\begin{itemize}
...
\subsection*{Язык, задаваемый грамматикой}
\begin{definition}
...
\begin{definition}
	$w$ выводимо в грамматике $G = \langle N,\, \Sigma,\, P,\, S\rangle$, если $S \vdash_G w$
\end{definition}
...
\subsection*{Иерархия Хомского порождающих грамматик}
...
\section{Про праволинейные грамматики}
\subsection*{Праволинейные языки}
\begin{theorem}
...
\subsection*{Построение праволинейной грамматики по конечному автомату}
...
\section{Построение конечного автомата по праволинейной грамматике}
Строим автомат:
\begin{align*}
	M = \langle S \cup \{q_f\},\, \Sigma,\, \Delta,\, S,\, \{q_f\}\rangle
\end{align*}
Переходы:
\begin{enumerate}
...
\begin{proof}
	...
	\begin{align*}
		\langle A,\, w\rangle \vdash_M \langle C,\, u\rangle \vdash_{M,\,1} \langle q_f,\, \varepsilon\rangle \Rightarrow A \vdash_G vC,\, C \vdash_G u \Rightarrow A \vdash_G vu = w
	\end{align*}
...
\section{Про КС-грамматики}
\subsection*{Примеры контекстно-свободных языков}
\begin{example}
	\begin{align*}
		S \to aSb \\
		S \to \varepsilon
...
\subsection*{Замкнутость КС-языков относительно простейших операций}
\begin{proposition}
...
\section{Удаление непорождающих и недостижимых символов в алгоритме примедения к нормальной форме Хомского. Асимптотика приведённых шагов.}
\subsection*{Удаление непорождающих символов}
\begin{definition}
	Символ $Y \in N$ называется порождающим, если:
	\begin{align*}
...
\begin{proof}
	Включение $L(G) \subset L(G_1)$ очевидно, так как мы удалили нетерминалы, которые никак не влияли на вывод слов, поэтому язык не уменьшился.

	Докажем $L(G_1) \subset L(G)$. Пусть $w \in L(G) \setminus L(G_1)$.
...
\subsection*{Удаление недостижимых символов}
\begin{definition}
	Символ $D \in N$ называется достижимым, если существуют некоторые $\phi,\, \psi \in (N \cup \Sigma)^*$, такие, что:
	\begin{align*}
		S \vdash \phi D \psi
	\end{align*}
\end{definition}
...
\begin{proof}
	Включение $L(G_1) \subset L(G_2)$ очевидно, так как удалили все нетерминалы, которые не участвовали в выводе, поэтому язык не уменьшился.

  Теперь докажем $L(G_2) \subset L(G_1) \Rightarrow \exists w \in L(G_1) \setminus L(G_2) \Rightarrow$ существует недостижимый $U :\: S \vdash \alpha U \beta \vdash w \Rightarrow U$ -- достижимый. Противоречие. 
\end{proof}
...
\subsection*{Асимптотика приведённых шагов (в терминах изначальной грамматики)}
\begin{enumerate}
...
\section{Удаление длинных, смешанных правил и eps-порождающих символов. Асимптотика приведённых шагов.}
\subsection*{Удаление длинных правил}
Сделаем замену:
\[
...
\subsection*{Удаление смешанных правил}
Сделаем замену:
\[
  A \to A_1bA_2d
\]
...
\subsection*{Удаление eps-порождающих}
\begin{definition}
...
\section{Обработка стартового состояния и удаление цепных правил. Асимптотика приведённых шагов.}
...
\section{Алгоритм Кока-Янгера-Касами синтаксического разбора для КС-грамматик}
...
\section{Корректность алгоритма Эрли}
\begin{definition}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item ПредположениеL $S' \vdash w[0:i]A\psi \vdash_1 w[0:i]\alpha B \beta\psi$
...
Корректность ситуации при наличии вывода гарантируется, тем, что:
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item Предположение $(j - 1,\, k,\, k+l):\: (A \to \alpha'\cdot bB,\, i) \in D_{j - 1}$
      \item $(A \to \alpha'b\cdot B,\, i) \in D_j$ по Scan
    \end{itemize}
    \item $\alpha = \alpha' B$
    \begin{itemize}
      \item $S' \vdash_{k + t + 1}w[0:p]B\beta\psi,\, B \vdash_{l - t - 1} w[p:j]$
      \item Предположение $(j,\, k + l,\, l - t - 1) :\: (B \to \gamma\cdot,\, p) \in D_j$
      \item $S' \vdash_k w[0:i]A\psi,\, \alpha' \vdash_t w[i:p]$
      \item Предположение $(p,\, k + t,\, t) :\: (A \to \alpha'\cdot B\beta\, i) \in D_p \Rightarrow (A \to \alpha'B\cdot\beta,\, i) \in D_j$ -- по Complete
    \end{itemize}
    \item $\alpha = \varepsilon$
    \begin{itemize}
      \item $S' \vdash w[0 : p]B\psi',\, \gamma \vdash_t w[p:i]$
      \item Предположение $(j,\, k - 1,\, t):\: (B \to \gamma \cdot A\delta,\, p) \in D_i,\, (A \to \beta) \in P \Rightarrow (A \to \cdot\beta,\, i) \in D_i$ -- по Predict
    \end{itemize}
  \end{enumerate}
\end{itemize}

\section{Оптимальный алгоритм и обоснование сложности}
...
\subsection*{Обоснование сложности}
